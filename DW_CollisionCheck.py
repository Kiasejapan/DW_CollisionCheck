...
def _es_expand_brim_from_vertex(shape, vert_idx, forbidden_edge_set):
    ...
    contains an edge incident to this vertex AND is NOT one of the
    selected rungs.

    Strategy: pick one incident edge that isn't in the forbidden set,
    then use `polySelect -edgeLoop` to grow that edge's loop.

    Returns a list of (edge_index, v0_idx, v1_idx) tuples describing
    the loop, in traversal order starting from the seed vertex, or
    [] if no suitable edge loop could be found.
    """
    if not MAYA_AVAILABLE:
        return []
    vtx = u"{0}.vtx[{1}]".format(shape, vert_idx)
    # Find edges incident to this vertex.
    try:
        ...
        return []
    # Pick a candidate edge not in the forbidden set (i.e. not a rung).
    seed_edge = None
    seed_idx = None
    for e in incident:
    ...
    if seed_edge is None:
        return []
    # Expand via polySelect -edgeLoop.
    saved_sel = cmds.ls(sl=True, fl=True, long=True) or []
    try:
        cmds.select(clear=True)
        try:
            cmds.polySelect(shape, edgeLoop=seed_idx, asSelectString=False)
        except Exception:
            try:
                cmds.select(seed_edge, r=True)
            except Exception:
                return []
        loop_edges = cmds.ls(sl=True, fl=True, long=True) or []
        loop_edges = [x for x in loop_edges if u".e[" in x]
    finally:
        try:
            cmds.select(saved_sel, r=True) if saved_sel else cmds.select(clear=True)
        except Exception:
            pass
    # Build ordered list: sequence of (edge_idx, v0, v1).
    ordered = []
    for e in loop_edges:
        try:
            ei = int(e.split(u"[")[-1].rstrip(u"]"))
            ...
            for v in verts:
                try:
                    vids.append(int(v.split(u"[")[-1].rstrip(u"]")))
                except Exception:
                    pass
            if len(vids) == 2:
                ordered.append((ei, vids[0], vids[1]))
        except Exception:
            continue
    return ordered
...
def _es_detect_belt_brims(edges):
    """For each rung edge in `edges`, detect the two brim edge loops
    (one touching side A, one touching side B) that the rung connects.

    Returns a dict:
        {
            "rung_to_brim_a": {rung_ei -> list of (edge_idx, v0, v1)},
            "rung_to_brim_b": {rung_ei -> list of (edge_idx, v0, v1)},
            "brim_a_verts":   ordered vertex positions for brim A per rung,
            "brim_b_verts":   ordered vertex positions for brim B per rung,
        }

    If any rung fails brim detection, its entry is None. Detection is
    per-rung because a selection may span multiple disconnected
    belts.
    """
    if not MAYA_AVAILABLE:
        return {"rung_to_brim_a": {},
                "rung_to_brim_b": {},
                "brim_a_verts":   {},
                "brim_b_verts":   {}}
    rung_edge_set = set(e["edge"] for e in edges)

    def _collect_verts_in_order(shape, ordered_edges):
        """Chain together ordered (edge, v0, v1) tuples into a sequence
        of connected vertex positions. Returns list of (x,y,z) or []
        on failure."""
        if not ordered_edges:
            return []
        if len(ordered_edges) == 1:
            _, v0, v1 = ordered_edges[0]
            try:
                p0 = cmds.pointPosition(
                    u"{0}.vtx[{1}]".format(shape, v0), w=True)
                p1 = cmds.pointPosition(
                    u"{0}.vtx[{1}]".format(shape, v1), w=True)
                return [(p0[0], p0[1], p0[2]),
                        (p1[0], p1[1], p1[2])]
            except Exception:
                return []
        # Build vertex adjacency from the edges.
        adj = {}
        for _, v0, v1 in ordered_edges:
            adj.setdefault(v0, set()).add(v1)
            adj.setdefault(v1, set()).add(v0)
        # Find endpoints (deg 1) — open loop; else closed.
        endpoints = [v for v, nbs in adj.items() if len(nbs) == 1]
        if len(endpoints) == 2:
            start = endpoints[0]
        elif len(endpoints) == 0:
            start = next(iter(adj))
        else:
            # Branching — can't linearize cleanly; bail.
            return []
        seq = [start]
        prev = None
        cur = start
    ...
    for ei, e in enumerate(edges):
        shape = e["shape"]
        brim_a_raw = _es_expand_brim_from_vertex(
            shape, e["a_idx"], rung_edge_set)
        brim_b_raw = _es_expand_brim_from_vertex(
            shape, e["b_idx"], rung_edge_set)
        rung_to_brim_a[ei] = brim_a_raw
        rung_to_brim_b[ei] = brim_b_raw
        brim_a_verts[ei] = _collect_verts_in_order(shape, brim_a_raw)
        brim_b_verts[ei] = _collect_verts_in_order(shape, brim_b_raw)
    return {"rung_to_brim_a": rung_to_brim_a,
            "rung_to_brim_b": rung_to_brim_b,
            "brim_a_verts":   brim_a_verts,
...
class _EsAngleHistogram(QtWidgets.QWidget):
    ...
    between adjacent selected edges (0°..90°), overlaid with the
    current corner-angle threshold.
    ...
    def paintEvent(self, _ev):
        ...
        # Chart area leaves a tiny 2px margin.
        pad = 2
        chart_w = w - pad * 2
        chart_h = h - pad * 2
        ...
        for i, c in enumerate(bins):
            if c == 0:
                continue
            bar_h = int((c / float(max_count)) * chart_h) if max_count > 0 else 0
            if bar_h < 1:
                bar_h = 1
            x0 = pad + int(i * bin_w)
            ...
            p.fillRect(x0, pad + chart_h - bar_h,
                       max(1, x1 - x0 - 1), bar_h, col)

        # Threshold line.
        pen = QtGui.QPen(QtGui.QColor("#FF9800"), 2)
        p.setPen(pen)
        p.drawLine(t_px, pad, t_px, pad + chart_h)

        # Baseline axis.
        p.setPen(QtGui.QPen(QtGui.QColor("#444"), 1))
        p.drawLine(pad, pad + chart_h, pad + chart_w, pad + chart_h)
        ...
        for deg in (0, 30, 60, 90):
            ...
            p.drawText(x - 8, pad + chart_h - 2 + 10,
                       u"{0}\u00B0".format(deg))
...
class EdgeSnapResultWindow(QtWidgets.QDialog):
    ...
    def _build(self):
        ...
        # Histogram of bend angles between adjacent edges. Updated
        # live on set_data and when the user drags the corner slider
        # so they can see exactly which pairs will be cut.
        self._angle_histogram = _EsAngleHistogram()
        self._angle_histogram.setFixedHeight(42)
    ...
    def _ensure_brim_data(self):
        ...
        if self._brim_data is not None:
            return
        if not self._edges:
            self._brim_data = {"rung_to_brim_a": {},
                                "rung_to_brim_b": {},
                                "brim_a_verts":   {},
                                "brim_b_verts":   {}}
            return
    ...
    def _on_corner_angle_changed(self, val):
        ...
        # Always update the histogram threshold — it's cheap and
        # should reflect the slider even when there are no edges yet.
        if hasattr(self, "_angle_histogram"):
            self._angle_histogram.set_threshold(self._corner_angle)
        if not self._edges:
            return
